...
### @property装饰器
用来托管类属性(https://blog.csdn.net/yingxuanzhang/article/details/125071269)。通过此装饰器可以访问私有属性，如果在访问、设置、删除属性时需要额外的操作，也可通过该装饰器，在类内添加相应的方法实现。
...
### 静态方法和类方法
当方法不需要访问对象属性也不需要访问类属性，可以定义为静态方法，用@staticmethod修饰，而且方法中不要求第一个参数（实例方法中需要第一个参数是self）。
...
通过cls这个参数我们可以获取和类相关的信息并且可以创建出类的对象，代码如下所示。
...
if __name__ == '__main__':
    main()
```
使用类方法可以创建实例对象，类似于c++中不同的构造函数，例如：
```
